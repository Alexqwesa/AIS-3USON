...
BEGIN
    ...
    DECLARE cursor_i CURSOR FOR
        select w.`user`, role_sqlname
        from kcson.dep_has_worker dhw
                 join kcson.`role` r on r.id = dhw.role_id
                 join kcson.worker w on dhw.worker_id = w.id
        where worker_id = get_WID()
          and dhw.dep_id = GET_DEP(get_WID())
          and w.`user` <> "root";
...
# for mariaDB
    ...
    LOOP
        ...
        IF drop_priv THEN
            ...
            SET @queryStringRP = CONCAT('REVOKE manager, worker, specialist, trusted_specialist  FROM  ', cursor_n,
                                        ';  '); # admin, part_admin
            PREPARE stmt FROM @queryStringRP;
            EXECUTE stmt;
            DEALLOCATE PREPARE stmt;
...
delimiter $$
create
    definer = `root`@`localhost` PROCEDURE `kcson`.`SET_DEP_`(`depId` INT)
    MODIFIES SQL DATA
...
-- 	   select  @queryStringRP;
        ...
        SET @queryStringRP = CONCAT(
                'REVOKE reporter, info, worker, specialist, trusted_specialist, manager, part_admin, admin, booker FROM  "',
...
delimiter $$
create
    definer = `root`@`localhost` PROCEDURE `kcson`.`show_cols_root`(IN tname TEXT)
    READS SQL DATA
    DETERMINISTIC
begin
...
BEGIN
    ...
    THEN
        ...
        SELECT SUM(((`f`.`perc` * `f`.`quantity`) * `f`.`price`)) AS to_pay,
               f.perc,
        ...
        from contracts c
                 JOIN `ripso` `r` ON ((`c`.`ripso_id` = `r`.`id`))
        where c.client_id = UID
...
$$
CREATE
    DEFINER = `root`@`localhost` PROCEDURE `kcson`.`replace_user`(IN `p_Name` VARCHAR(16), IN `p_Passw` VARCHAR(32), IN wID int)
this_proc:
BEGIN
    ...
    set crole := (select dhw.role_id
                  from dep_has_worker dhw
                           inner join worker w on
                      w.id = dhw.worker_id
                  where w.`user` = cuser
                    and (dhw.role_id in (6, 7, 8))
                  order by dhw.role_id desc
                  limit 1);
    -- select crole;

    -- get role of user whom we trying to change here
    set wrole := (select dhw.role_id
                  from dep_has_worker dhw
                  where dhw.id = wID
                  order by dhw.role_id desc
                  limit 1);
